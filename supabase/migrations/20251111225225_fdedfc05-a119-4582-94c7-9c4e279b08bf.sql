-- Fase 1: Actualizar función seed_role_permissions para incluir módulo 'crm'
...
BEGIN
  ...
  IF p_role_name = 'admin' THEN
    ...
    VALUES
      ...
      (p_user_id, 'crm', true, true, true, true),
  ...
  ELSIF p_role_name = 'colaborador' THEN
    ...
    VALUES
      ...
      (p_user_id, 'crm', true, true, true, false),
...
-- Fase 2: Insertar módulo 'crm' para usuarios existentes con roles admin y colaborador
INSERT INTO public.user_permissions (user_id, module_name, can_view, can_create, can_edit, can_delete)
SELECT 
  ur.user_id,
  'crm',
...
SELECT 
  ...
  false
FROM public.user_roles ur
WHERE ur.role_name = 'colaborador'
ON CONFLICT (user_id, module_name) DO NOTHING;