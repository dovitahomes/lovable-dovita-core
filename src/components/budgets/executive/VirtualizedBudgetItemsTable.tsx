...
import { ExecutiveItemDialog } from "./ExecutiveItemDialog";
...
export function VirtualizedBudgetItemsTable({
  ...
  const handleSaveItem = (updatedItem: ExecutiveBudgetItem) => {
    if (editingItem) {
      onUpdateItem(editingItem.index, "descripcion", updatedItem.descripcion);
      onUpdateItem(editingItem.index, "unidad", updatedItem.unidad);
      onUpdateItem(editingItem.index, "cant_real", updatedItem.cant_real);
      onUpdateItem(editingItem.index, "desperdicio_pct", updatedItem.desperdicio_pct);
      onUpdateItem(editingItem.index, "costo_unit", updatedItem.costo_unit);
      onUpdateItem(editingItem.index, "honorarios_pct", updatedItem.honorarios_pct);
      onUpdateItem(editingItem.index, "proveedor_alias", updatedItem.proveedor_alias);
      setEditingItem(null);
    }
  };
          ...
          <Table>
            <TableHeader className="sticky top-0 bg-background z-10 shadow-sm">
              <TableRow>
                ...
                <TableHead className="w-[80px] sticky right-0 bg-background">
              ...
              {virtualItems.map((virtualRow) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Total */}
                    <TableCell className="min-w-[120px] text-right sticky right-20 bg-background">
                      <span className={cn(
                    ...
                    {/* Actions */}
                    <TableCell className="w-[80px] sticky right-0 bg-background">
                      <div className="flex items-center justify-end gap-1">
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => setEditingItem({ item, index: globalIndex })}
