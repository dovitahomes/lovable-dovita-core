...
interface BudgetItem {
  ...
  unidad: string;
  cant_real: number;
  desperdicio_pct?: number; // Opcional (solo para colaboradores)
  costo_unit?: number; // Opcional (solo para colaboradores)
  honorarios_pct?: number; // Opcional (solo para colaboradores)
  proveedor_alias?: string; // Opcional (solo para colaboradores)
}
...
export function BudgetItemRow({
  ...
  const canEdit = can("presupuestos", "edit");
  const canDelete = can("presupuestos", "delete");
  ...
  // Calcular con valores por defecto si no están disponibles (modo cliente)
  const desperdicio = item.desperdicio_pct ?? 0;
  const costoUnit = item.costo_unit ?? 0;
  const honorarios = item.honorarios_pct ?? 0;
  
  const cantNecesaria = item.cant_real * (1 + desperdicio / 100);
  const precioUnit = costoUnit * (1 + honorarios / 100);
  ...
  // Vista completa (solo para colaboradores/admin)
  if (!clienteView && canEdit) {
    return (
      <div className="grid grid-cols-12 gap-2 p-2 hover:bg-muted/50 rounded-lg group text-sm border-b">
        ...
        <div className="text-center text-xs">{desperdicio}%</div>
        <div className="text-right text-xs">{cantNecesaria.toFixed(2)}</div>
        <div className="text-right text-xs">
          ${costoUnit.toLocaleString('es-MX', { minimumFractionDigits: 2 })}
        </div>
        <div className="text-center text-xs">{honorarios}%</div>
        <div className="text-right text-xs">
  ...
  // Vista cliente (sin columnas sensibles)
  return (
    <div className="grid grid-cols-8 gap-2 p-2 hover:bg-muted/50 rounded-lg text-sm border-b">
      <div className="col-span-2">
        <div className="font-medium">{mayor?.name}</div>
