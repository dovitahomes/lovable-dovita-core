...
import { Handshake, CheckCircle2, Eye, FileDown, DollarSign } from "lucide-react";
...
import { PaymentTimeline } from "@/components/commissions/PaymentTimeline";
...
import { exportCommissionsToExcel, exportCommissionsToPDF } from "@/utils/exports/commissionExports";
...
export default function ComisionesAlianzas() {
  ...
  const [paymentCommission, setPaymentCommission] = useState<any>(null);
  ...
  const handleExportExcel = async () => {
    if (!commissions || commissions.length === 0) {
      toast({
        title: "Sin datos",
        description: "No hay comisiones para exportar",
        variant: "destructive",
      });
      return;
    }
    ...
    const exportData = commissions.map((c: any) => ({
      id: c.id,
      tipo: c.tipo,
      sujeto_nombre: alliances?.find(a => a.id === selectedAllianceId)?.nombre || "Desconocido",
      cliente_nombre: c.budgets?.projects?.clients?.name || "Sin cliente",
      proyecto_nombre: c.budgets?.projects?.name || "Sin proyecto",
    ...
    try {
      await exportCommissionsToExcel(exportData, filters);
      toast({
        title: "Éxito",
  ...
  const handleExportPDF = async () => {
    ...
    try {
      await exportCommissionsToPDF(exportData, filters);
      toast({
        title: "Éxito",
  ...
  const handleBulkMarkAsPaid = () => {
    if (selectedCommissionIds.length > 0) {
      const total = commissions
        ...
        .reduce((sum: number, c: any) => sum + c.calculated_amount, 0);

      if (
        confirm(
          `¿Marcar ${selectedCommissionIds.length} comisiones como pagadas?\nTotal: $${total?.toLocaleString(
            "es-MX",
  ...
  return (
    <div className="space-y-6 max-w-full overflow-x-hidden px-4 sm:px-6 py-6">
      ...
      {selectedAllianceId && (
        <Tabs defaultValue="comisiones" className="animate-fade-in">
          <Card>
            <CardHeader>
                                  ...
                                  <div className="flex justify-end gap-2">
                                    ...
                                    {commission.status !== 'pagada' && (
                                      <Button
                                        variant="outline"
                                        size="sm"
                                        onClick={() => setPaymentCommission(commission)}
                                      >
            ...
            <TabsContent value="pagos">
              <CardContent>
                <PaymentTimeline alianzaId={selectedAllianceId} />
              </CardContent>
            </TabsContent>
          </Card>
        </Tabs>
      ...
      {paymentCommission && (
        <PaymentDialog
          open={!!paymentCommission}
          onOpenChange={(open) => !open && setPaymentCommission(null)}
